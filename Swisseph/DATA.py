...
special_positions = {
    'Moon_North_Node': 'Moon_North_Node_ELONG',
    'Moon_South_Node': 'Moon_South_Node_ELONG',
    'Lilith': 'Lilith_ELONG',
    'Selena': 'Selena_ELONG',
    'ASC': 'ASC',
    'MC': 'MC',
...
for item, pos in special_positions.items():
    all_data_df[[f"{item}_Zodiac_signs", f"{item}_Zodiac_sign_element", f"{item}_Zodiac_degree"]] = all_data_df[pos].apply(zodiac_sign).apply(pd.Series)
    all_data_df[[f"{item}_Mansion_positions", f"{item}_Mansion_degree", f"{item}_Mansion_element"]] = all_data_df[pos].apply(mansion_position)
    all_data_df[[f"{item}_Astronomy_boundary", f"{item}_Astronomy_degree"]] = all_data_df[pos].apply(real_astronomy_boundaries)
...
all_data_df.to_csv("/Users/x/EXPORT_ALL_DATA_Y2006to2023_2.csv", index=False)
