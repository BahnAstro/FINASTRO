...
readdata1 = pd.read_csv("/Users/x/HSI_HL_2006to2023_updated.csv", header=None, skiprows=1, names=['DateTime', 'Value', 'Type'], encoding='ISO-8859-1')
...
# 24 Solar Terms
    ...
    def lunar_month_and_elements(ecl_lon):
            months = [
            (315, 345, '寅'),
            (345, 360, '卯上'),
            (0, 15, '卯下'),
            ...
            for i, (start, end, month) in enumerate(months):
                if start > end:
                    if ecl_lon >= start or ecl_lon < end:
                        return month, elements_order[i]
                else:
                    if ecl_lon >= start and ecl_lon < end:
                        return month, elements_order[i]
            return None, None
...
#農曆月及五行
all_data_df['Lunar_month'], all_data_df['Elements'] = zip(*all_data_df['Sun_ELONG'].apply(lunar_month_and_elements))
...
for planet, pos in planet_positions.items():
    all_data_df[[f"{planet}_ELONG_Zodiac_signs", f"{planet}_ELONG_Zodiac_degree"]] = all_data_df[pos].apply(zodiac_sign).apply(pd.Series)

all_data_df[['Moon_North_Node_ELONG_Zodiac_signs', 'Moon_North_Node__ELONG_Zodiac_degree']] = all_data_df['Moon_North_Node_ELONG'].apply(zodiac_sign)
all_data_df[['Moon_South_Node_ELONG_Zodiac_signs', 'Moon_South_Node_ELONG_Zodiac_degree']] = all_data_df['Moon_South_Node_ELONG'].apply(zodiac_sign)
all_data_df[['Lilith_ELONG_Zodiac_signs', 'Lilith_ELONG_Zodiac_degree']] = all_data_df['Lilith_ELONG'].apply(zodiac_sign)
all_data_df[['Selena_ELONG_Zodiac_signs', 'Selena_ELONG_Zodiac_degree']] = all_data_df['Selena_ELONG'].apply(zodiac_sign)
...
# 獲取行星所在的宮位和度數
for planet, pos in planet_positions.items():
    all_data_df[[f"{planet}_ELONG_Mansion_positions", f"{planet}_ELONG_Mansion_degree"]] = all_data_df[pos].apply(mansion_position)


all_data_df[['Moon_North_Node_ELONG_Mansion_positions', 'Moon_North_Node_ELONG_Mansion_degree']] = all_data_df['Moon_North_Node_ELONG'].apply(mansion_position)
all_data_df[['Moon_South_Node_ELONG_Mansion_positions', 'Moon_South_Node_ELONG_Mansion_degree']] = all_data_df['Moon_South_Node_ELONG'].apply(mansion_position)
all_data_df[['Lilith_ELONG_Mansion_positions', 'Lilith_ELONG_Mansion_degree']] = all_data_df['Lilith_ELONG'].apply(mansion_position)
all_data_df[['Selena_ELONG_Mansion_positions', 'Selena_ELONG_Mansion_degree']] = all_data_df['Selena_ELONG'].apply(mansion_position)
